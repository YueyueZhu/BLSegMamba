...
class ECALayer(nn.Module):
    ...
    def __init__(self, channel, k_size=3):
        ...
        self.conv = nn.Conv3d(self.channel, 1, kernel_size=(k_size, 1, 1), padding=((k_size - 1) // 2, 0, 0), bias=False)
    ...
    def forward(self, x):
        ...
        y = self.conv(y).squeeze(-1).squeeze(-1).squeeze(-1)
...
class UKAN(nn.Module):
    def __init__(self,input_channnel, num_classes, embed_dims=[128, 160, 256], drop_rate=0.1, drop_path_rate=0.1, norm_layer=nn.LayerNorm, depths=[1, 1, 1]):
        super().__init__()

        kan_input_dim = embed_dims[0]

        self.encoder1 = D_ConvLayer3D(input_channnel, kan_input_dim//8)
...
if __name__ == '__main__':
    ...
    model = UKAN(input_channnel=1,num_classes=2)
    # input_tensor = torch.randn(1, 1, 100, 128, 128).to(device)
