...
class MambaLayer(nn.Module):
    def __init__(self, dim, stage=1, d_state = 16, d_conv = 4, expand = 2, pos_embed=True, in_shape=[128, 128, 128],num_slices=None):
        super().__init__()
        self.dim = dim
...
class EMNetUpBlock(nn.Module):
    ...
    def __init__(
        ...
        if res_block:
            stage_blocks = []
            num_slices_list = [32, 16, 8]
            for i in range(3):
                stage_blocks.append(MambaLayer(dim=out_channels, stage=stage-1, 
                                               d_state=16, d_conv=4, expand=2, 
                                               pos_embed=True, in_shape=in_shape,num_slices=num_slices_list[i]))
